...
The live compose/image/volume layout is now inspected and no production Git worktree was found. The private [psitrends-ops repository](https://github.com/andylitvinov-design/psitrends-ops) now versions sanitized nginx/PHP definitions, guarded backup tooling and systemd units. Private data backups stay outside Git; the live environment/compose export is not uploaded. Client content and the native Joomla template adapter remain in sales. A production-image restore clone is isolated on the host, with SSH-only browser access; cache/session isolation differs deliberately from Redis production settings.
