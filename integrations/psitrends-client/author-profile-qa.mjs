...
const base=live?'https://psitrends.com':'http://127.0.0.1:8879/sales';
const browser=await chromium.launch({channel:'chrome',headless:true});
const results=[];
try {
 for(const width of [320,390,768,1440]){
  const context=await browser.newContext({...width<768?devices['iPhone 13']:{},viewport:{width,height:1000}});
  ...
  for(const locale of ['en','ru'])for(const key of Object.keys(pages[locale])){
   ...
   const url=live?base+routeFor(key,locale):`${base}/psitrends-client-${key}${locale==='ru'?'-ru':''}.html`;
   ...
   if(['about','home'].includes(key)){
    ...
    if([390,1440].includes(width))await page.screenshot({path:`reports/author-${live?'live':'local'}-${locale}-${key}-${width}.png`});
   }
   results.push({locale,key,width,...m});await page.close();
  }
  await context.close();
 }
 await fs.writeFile(`reports/author-profile-${live?'live':'local'}.json`,JSON.stringify({ok:true,checks:results.length,results},null,2));
