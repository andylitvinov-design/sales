...
export function render(key,locale,{source=false,production=false}={}){
 const c=key==='home'?{...pages[locale][key],...home[locale]}:pages[locale][key],s={...shared[locale],...{final:home[locale].final,finalText:home[locale].finalText}};
 const link=(k,l=locale)=>source?`${sourceName(k,l)}.html`:routeFor(k,l);
 const languageLabel=l=>locale==='en'?(l==='en'?'English, current language':'Switch to Russian'):(l==='ru'?'Русский, текущий язык':'Переключить на английский');
 const languageSwitch=['ru','en'].map(l=>l===locale?`<span class="language current-language" lang="${l}" aria-current="page" title="${esc(languageLabel(l))}">${l.toUpperCase()}</span>`:`<a class="language" href="${link(key,l)}" lang="${l}" hreflang="${l}" aria-label="${esc(languageLabel(l))}">${l.toUpperCase()}</a>`).join('');
...
<header class="site-header"><div class="shell header-top"><a class="brand" href="${link('home')}">PsiTrends<span>Holistic House</span></a><nav class="nav" id="primary-nav" aria-label="${esc(s.nav)}">${navItems.map(([id,label])=>`<a href="${homeAnchor(id)}">${label}</a>`).join('')}${['about','contact'].map(k=>`<a href="${link(k)}"${k===key?' aria-current="page"':''}>${locale==='en'&&k==='about'?'About':esc(s.labels[k])}</a>`).join('')}</nav><div class="header-controls">${languageSwitch}<button class="menu-toggle" type="button" aria-controls="primary-nav" aria-expanded="false" aria-label="${locale==='en'?'Open menu':'Открыть меню'}"><span></span><span></span></button></div></div></header>
<main id="main">
